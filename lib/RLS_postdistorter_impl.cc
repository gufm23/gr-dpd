      ...
      d_iter_limit(iter_limit)
{
    ...
    L_bar_iMinus1 =
        zeros<cx_mat>(K_a * (L_a + 1) + K_b * M_b * (L_b + 1), (K_a + K_b * M_b) * 2);
    L_bar_iMinus1(span(0, K_a * (L_a + 1) - 1), span(0, K_a * 2 - 1)) = L_bar_iMinus1_a;
    L_bar_iMinus1(span(K_a * (L_a + 1), K_a * (L_a + 1) + K_b * M_b * (L_b + 1) - 1),
                  span(K_a * 2, (K_a + K_b * M_b) * 2 - 1)) = L_bar_iMinus1_b;
...
void RLS_postdistorter_impl::givens_rotate(const cx_mat& in, cx_mat& out)
{
    ...
    if (almost_equal(b, 0.0, 100.0)) {
        ...
        scale = a + b;
        norm =
            scale * sqrt(pow(abs(in(0, 0) / scale), 2) + pow(abs(in(0, 1) / scale), 2));
        alpha = in(0, 0) / a;
        theta(0, 0) = a / norm;
        theta(1, 1) = theta(0, 0);
...
void RLS_postdistorter_impl::hgivens_rotate(const cx_mat& in, cx_mat& out)
{
    ...
    if (almost_equal(abs(x), 0.0, 100.0)) {
        ...
        // Fast Reliable Algorithms for Matrices with Structure - Edited by Sayed and
        // Kailath
        for (int ii = 0; ii < in.n_rows; ii++) {
            if (almost_equal(abs(in(ii, 0)), 0.0, 100.0) &&
                almost_equal(abs(in(ii, 1)), 0.0, 100.0)) {
                out(ii, 0) = in(ii, 0);
                out(ii, 1) = in(ii, 1);
            } else {
                x = in(ii, 0);
...
void RLS_postdistorter_impl::extract_g_vecs(cx_mat& g,
...
void RLS_postdistorter_impl::gen_GMPvector(const gr_complex* const in,
                                           int item,
                                           int K_a,
                                           int L_a,
                                           int K_b,
...
int RLS_postdistorter_impl::work(int noutput_items,
